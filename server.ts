...
// Scrape website content via Jina Reader (free, no API key needed)
async function scrapeWebsite(url: string): Promise<string> {
  ...
  try {
    ...
    // Truncate to ~6000 chars to stay within token limits while keeping useful content
...
// AI Readiness Calculator endpoint
app.post('/api/ai-readiness', rateLimit, async (req, res) => {
  try {
    if (!openai) {
